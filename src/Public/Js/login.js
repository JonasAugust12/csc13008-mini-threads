...
function checkInputs() {
    if (emailOrUsernameInput.value.trim() !== '' && passwordInput.value.trim() !== '') {
...
document.querySelector('.sign-up__text-wrapper').addEventListener('click', () => {
    window.location.href = '/go-to-signup';
});
...
document.getElementById('login_form').addEventListener('submit', async function (event) {
    ...
    try {
        ...
        const errorMessage = error.response?.data?.message || 'Login failed! Something went wrong.';
